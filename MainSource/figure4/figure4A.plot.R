...
datarow = read.table('figure4A.row.list',header = TRUE,sep = "\t",stringsAsFactors=TRUE)
data = read.table('figure4A.list',header = TRUE,sep = "\t",stringsAsFactors=TRUE)
dat=data[,0:-1]
test=matrix(unlist(dat), ncol = 48, nrow =24)
...
annotation_col = data.frame( Sample_Clade = factor(rep(c("19A","19B","20A","20B","20C"), c(1,17,2,27,1))))  # {'19A': 1, '19B': 17,'20A': 2,'20B': 27,  '20C': 1}
...
pheatmap(test,cluster_row = FALSE,cluster_col = FALSE,angle_col = "90",
         ...
         #annotation_col = annotation_col,gaps_col = c(1,18,20,47),
